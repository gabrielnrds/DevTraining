...
public class PrimaryController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        configurarBotoes(BtnLogin);
        configurarBotoes(BtnRegistro);
    }
    public static void configurarBotoes(Button button) {
        button.setOnMouseEntered(event -> {
            button.setStyle("-fx-background-color: #f78120; -fx-border-color: #ADD8E6;");
        });

        button.setOnMouseExited(event -> {
            button.setStyle("-fx-background-color: #f78120; -fx-border-color: black;");
        });

        button.setOnMousePressed(event -> {
            button.setStyle("-fx-background-color: #f78120; -fx-border-color: black; -fx-opacity: 0.7;");
        });
